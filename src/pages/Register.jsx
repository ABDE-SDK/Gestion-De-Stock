...
import { register ,loginUser} from '../Slices/authSlice';
  ...
  <div className='bg-white p-6 shadow-lg w-full max-w-sm'>
    ...
    <form className='space-y-2' onSubmit={(e)=>handleRegister(e)}>
      <div>
        ...
        <input type='text' name="username" placeholder='votre_username' onChange={validateUsername} className={`w-full border rounded px-3 py-1.5 text-sm focus:outline-none ${invalidUsername ? 'border-red-500' : 'border-gray-300 focus:border-green-500'}`} required />
      ...
      <div>
        ...
        <input type='email' name="email" placeholder='votre@email.com' onChange={validateEmail} className={`w-full border rounded px-3 py-1.5 text-sm focus:outline-none ${invalidEmail ? 'border-red-500' : 'border-gray-300 focus:border-green-500'}`} required />
      ...
      <div>
        ...
        <input type='password' name="password" placeholder='Min 8 caractères' onChange={validatePass} className={`w-full border rounded px-3 py-1.5 text-sm focus:outline-none ${invalidPass ? 'border-red-500' : 'border-gray-300 focus:border-green-500'}`} required />
      ...
      <div>
        ...
        <input type='password' name='confirm_pass' placeholder='Confirmer' onChange={verifyEqualPass} className={`w-full border rounded px-3 py-1.5 text-sm focus:outline-none ${inequalPass ? 'border-red-500' : 'border-gray-300 focus:border-green-500'}`} required />
        <div className='min-h-4 mt-1'>{inequalPass && <span className='text-red-600 text-xs block leading-tight'>{inequalPass}</span>}</div>
      </div>
      <div className='pt-1'>
        <button type='submit' className='w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded font-medium text-sm'>S'inscrire</button>
      </div>
    </form>
    <p className='text-sm text-gray-600 text-center mt-4'>Déjà inscrit ? <Link to="/login" className='text-green-600 hover:text-green-700 font-medium'>Se connecter</Link></p>
  </div>
</div>
  );
};
